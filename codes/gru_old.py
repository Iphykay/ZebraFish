...
def main():
    ...
    # Use the LSTM
    lstm_ntwrk, assessment, assessment2, conf_mtrx = gru_output(data, data[0].shape[2], useinCV, 'GRU', bandID=None, name=None, 
                                                                withrepr=False, reprtype='GRU', data3D_type=data3D_type, pathGrp='Trial01') 
#


def gru_model(Xtrain, Ytrain, bandID, num_feats, withrepr, reprtype, data3D_type=bool,
              model_name='GRU', pathGrp=str):
...
def gru_learner(num_feats, Xtrain, Ytrain, Xtest, Ytest, model_name, bandID, withrepr,
                reprtype, data3D_type=bool, pathGrp=str):
    
    ntwrk, best_hps, optimizer, batch_size, num_epochs = gru_model(Xtrain, Ytrain, bandID, num_feats, withrepr, reprtype,
                                                                   data3D_type=data3D_type, model_name='GRU', pathGrp=pathGrp)
...
def gru_output(img_data, num_feats, useinCV, model_name, bandID=None, name=None, withrepr=bool, 
    ...
    # Learner
    rnn_ntwrk, best_hps, history, pred_proba, pred_cls = gru_learner(num_feats, Xtrain, Ytrain, Xtest, Ytest, model_name, bandID, withrepr,
    ...
    # Model Assessement
    assessment, assessment2, conf_mtrx = rnn.model_assessment(Ytest, pred_cls, pred_proba, name, 'GRU', bandID, useinCV, 
                                                          plot=True)
